...
import group.xuxiake.entity.show.FilePathStore;
import group.xuxiake.handler.LowKbpsHandler;
import group.xuxiake.handler.RecycleHandler;
import group.xuxiake.handler.VideoTransformHandler;
import group.xuxiake.mapper.RecycleMapper;
...
public class FileUploadServiceImpl implements FileUploadService {
	...
	@Value("#{conf.fileSizeMax}")
	private Long fileSizeMax;
	@Value("#{conf.vipFileSizeMax}")
	private Long vipFileSizeMax;
	@Value("#{fd.nginx_server}")
	private String nginxServer;
	@Value("#{conf.recycleDelayTime}")
	private Long recycleDelayTime;
	...
	public Result uploadMD5(FileUpload fileUpload) {
		...
		try {
			...
			if (Integer.valueOf(userNetdisk.getUserStatus()) == NetdiskConstant.USER_STATUS_NORMAL) {
				if (Long.valueOf(fileUpload.getFileSize()) > fileSizeMax) {
			...
			if (Integer.valueOf(userNetdisk.getUserStatus()) == NetdiskConstant.USER_STATUS_VIP) {
				if (Long.valueOf(fileUpload.getFileSize()) > vipFileSizeMax) {
					result.setCode(NetdiskErrMsgConstant.CHECKMD5_OUT_OF_MEMORY_LIMIT);
					result.setMsg(NetdiskErrMsgConstant.getErrMessage(NetdiskErrMsgConstant.CHECKMD5_OUT_OF_MEMORY_LIMIT));
					return result;
				}
		...
		try {
			...
			if (fileType==2) {
				...
				if (fileSize>=100*1024) {
					InputStream is = FastDFSUtils.getInputStream(filePath);
					fastDFSUtils = new FastDFSUtils("classpath:FastDFS.properties");
					ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
					Thumbnails.of(is).size(500, 500).toOutputStream(byteArrayOutputStream);
					mediaCachePath = fastDFSUtils.uploadFile(byteArrayOutputStream.toByteArray(), fileExtName);
				}else {
					mediaCachePath = filePath;
				}
				mediaCachePath = nginxServer + "/" + mediaCachePath;
			...
			if (fileType == 4) {
				...
				InputStream is = FastDFSUtils.getInputStream(fileUpload.getFilePath());
				...
				new Thread(new VideoTransformHandler(fileUploadMapper, paths, fileUpload, nginxServer)).start();
			}
			if ("mp3".equals(fileExtName)) {
				...
				new Thread(new LowKbpsHandler(fileUploadMapper, paths, fileUpload, nginxServer)).start();
			}
	...
	public Result deleteFile(String fileSaveName) {
		...
		long delayTime = recycleDelayTime;
	...
	public void downLoad(FileUpload fileUpload) {
		...
		BufferedInputStream bis = null;
		try {
			bis = new BufferedInputStream(FastDFSUtils.getInputStream(filePath));
